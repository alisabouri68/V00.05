...
/**
 * v1.1 - Selection counter badge ("current / total")
 */
const CounterBadge = memo(({ text }: { text: string }) => (
  <span className="shrink-0 rounded-md bg-cyan-50  px-2 py-1 text-[10px] font-bold leading-none tabular-nums text-primary">
    {text}
  </span>
));
...
const DropdownLarge = memo(function DropdownLarge({
  ...
  const renderTriggerContent = () => {
    if (selectedOptions.length === 0) {
      return (
        <span className="truncate text-neutral-placeholder">
          {placeholder}
        </span>
      );
    }
    ...
    if (!multiple) {
      ...
      return (
        <span className="flex min-w-0 items-center gap-1 truncate">
          ...
          {showCounter && (
            <CounterBadge text={`${selectedIndex}/${options.length}`} />
          )}
        </span>
      );
    }
    ...
    return (
      <div className="flex min-w-0 flex-wrap items-center gap-1">
        {showCounter && (
          <CounterBadge text={`${selectedOptions.length}/${options.length}`} />
        )}
  ...
  return (
    ...
    >
      <button
        ...
        className={`
          flex w-full items-center justify-between gap-2 rounded-xl border px-3 transition-all
          ${geo?.height ? "" : "h-10"} 
          ${geo?.height ? "" : "h-10"} 
          ...
          ${
            bordered
              ...
              : `
                ...
                ${isOpen ? "bg-gray-100/70 dark:bg-gray-800/60" : ""}
              `
          }
          ${triggerClassName || ""}
        `}
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {indicatorIcon === "dots" && (
            <More size={16} className="stroke-gray-500 dark:stroke-gray-400 rotate-90" />
          )}
        </div>
      </button>
