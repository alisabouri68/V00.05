...
Title:          Large Dropdown Component - Optimized v1.0
Version:        01.00.00
VAR:            01

last-update:    D2026.07.16
owner:          apps68
  ...
  ({
    ...
    return (
      <div
        ...
        className={`flex items-center gap-2 px-3 cursor-pointer select-none transition-colors duration-100
          ${option.disabled ? "opacity-40 cursor-not-allowed" : ""}
          ${isHighlighted && !option.disabled ? "bg-gray-50 dark:bg-gray-700/60" : ""}
        `}
        style={{
          height: itemHeight,
...
const DropdownLarge = memo(function DropdownLarge({
  ...
  const flatRows = useMemo<FlatRow[]>(() => {
    ...
    groups.forEach((opts, groupLabel) => {
      rows.push({ type: "group", key: `group-${groupLabel}`, label: groupLabel });
      opts.forEach((opt) =>
  ...
  const selectableRows = useMemo(
    () => flatRows.filter((r): r is Extract<FlatRow, { type: "option" }> =>
  ...
  const rowHeights = useMemo(
    () => flatRows.map((r) => (r.type === "group" ? groupHeaderHeight : virtualItemHeight)),
  ...
  const bottomPad =
    rowOffsets.total - (rowOffsets.offsets[visibleRange.end] ?? rowOffsets.total);
  ...
  const renderMenu = () => {
    ...
    return createPortal(
      <div
        ...
        className="fixed rounded-xl border shadow-lg overflow-hidden flex flex-col"
        style={{
      ...
      >
        {searchable && (
          <div className="flex items-center gap-2 border-b px-3 py-2 shrink-0" style={{ borderColor }}>
            <SearchNormal1
              size={14}
        ...
        {multiple && selectAllOption && selectableRows.length > 0 && (
          <button
            ...
            className="flex items-center gap-2 border-b px-3 h-9 shrink-0 text-left hover:bg-gray-50 dark:hover:bg-gray-700/60 transition-colors"
            style={{ borderColor }}
          >
            <span
              className={`flex h-4 w-4 shrink-0 items-center justify-center rounded border
  ...
  return (
    ...
    >
      <button
        ...
        className={`flex w-full items-center justify-between gap-2 rounded-xl border px-3 transition-all
          ${compact ? "h-8" : "h-10"}
          ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer hover:border-cyan-400 dark:hover:border-cyan-500"}
          ${isOpen ? "border-cyan-500 ring-2 ring-cyan-100 dark:ring-cyan-900/40" : ""}
        `}
        style={{
          backgroundColor: triggerBg,
          borderColor: isOpen ? undefined : borderColor,
          color: textColor,
        }}
      >
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          {isOpen ? (
            <ArrowUp2 size={16} className="stroke-gray-500 dark:stroke-gray-400" />
          ) : (
            <ArrowDown2 size={16} className="stroke-gray-500 dark:stroke-gray-400" />
          )}
        </div>
      </button>
